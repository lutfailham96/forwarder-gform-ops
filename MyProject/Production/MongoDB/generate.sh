...
check_existing_forwarder() {
  if kubectl get svc -n ${PROJECT_NAMESPACE} | grep -q 27017; then
    SVC_NAME=$(kubectl get svc -n ${PROJECT_NAMESPACE} | grep 27017 | awk '{print $1}')
    FORWARDER_NAME=${SVC_NAME}
