...
PROJECT_NAMESPACE=tds-prod
SECRET_NAME=forwarder-postgresql
SVC_NAME=$(echo "tds-service-$(cat /dev/urandom | tr -dc 'a-z' | head -c 10)")
FORWARDER_NAME=${SVC_NAME}
UNTIL="21:00"

export KUBECONFIG=./my-kube-config.yaml
WORKDIR=./MyProject/Production/PostgreSQK
cd ${WORKDIR}
...
check_existing_forwarder() {
  if kubectl get svc -n ${PROJECT_NAMESPACE} | grep -v airflow | grep -q 5433; then
    SVC_NAME=$(kubectl get svc -n ${PROJECT_NAMESPACE} | grep -v airflow | grep 5433 | awk '{print $1}')
    FORWARDER_NAME=${SVC_NAME}
