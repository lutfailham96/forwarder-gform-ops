...
SECRET_NAME=forwarder-kafka
SVC_NAME=$(echo "my-project-$(cat /dev/urandom | tr -dc 'a-z' | head -c 10)")
FORWARDER_NAME=${SVC_NAME}
UNTIL="21:00"

export KUBECONFIG=./my-kube-config.yaml
WORKDIR=./MyProject/Production/Kafka
cd ${WORKDIR}
...
check_existing_forwarder() {
  if kubectl get svc -n ${PROJECT_NAMESPACE} | grep -v airflow | grep -q 9094; then
    SVC_NAME=$(kubectl get svc -n ${PROJECT_NAMESPACE} | grep 9094 | awk '{print $1}')
    FORWARDER_NAME=${SVC_NAME}
